...
const SCRIPT_NAME = "backfill_show_poster_paths";
...
async function ensureTables() {
  ...
  if (!showTable?.[0]?.table_name || !scriptLogsTable?.[0]?.table_name) {
    throw new Error("Required tables missing: expected public.show and public.script_logs.");
  }
}
...
async function fetchShowsNeedingPosters(limit) {
...
async function fetchTmdbShowPoster(apiKey, tmdbId) {
  const url = new URL(`${TMDB_TV_URL}/${tmdbId}`);
...
async function updateShowPosterPath(id, posterPath) {
  await sequelize.query(
    `
      UPDATE show
...
export async function backfillShowPosterPaths({ limit = null, apiKey } = {}) {
  const resolvedKey = apiKey ?? getApiKey();
  const rows = await fetchShowsNeedingPosters(limit);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      const data = await fetchTmdbShowPoster(resolvedKey, tmdbId);
      const posterPath = typeof data.poster_path === "string" ? data.poster_path : null;
      await updateShowPosterPath(id, posterPath);
      updated += 1;
      console.log(`${prefix} Show tmdb_id=${tmdbId} -> poster_path=${posterPath}`);
    } catch (error) {
      failed += 1;
      failedItems.push({ entityType: "show", tmdbId });
      console.warn(`${prefix} Show tmdb_id=${tmdbId} failed: ${error.message}`);
    }
  }
...
async function main() {
  ...
  try {
    try {
      ...
      const result = await backfillShowPosterPaths({ limit });

      console.log(
        `Show poster backfill complete. Total: ${result.total}. Updated: ${result.updated}. Failed: ${result.failed}.`
      );
...
if (isDirectRun) {
  main().catch((error) => {
    console.error("Failed to backfill show poster paths:", error.message);
    process.exit(1);
  });
}
