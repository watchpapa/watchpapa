...
const SCRIPT_NAME = "inject_top_rated_movies";
const TMDB_TOP_RATED_MOVIES_URL = "https://api.themoviedb.org/3/movie/top_rated";
...
async function fetchTopRatedMoviePage(apiKey, page) {
  const url = new URL(TMDB_TOP_RATED_MOVIES_URL);
  ...
  if (!response.ok) {
    ...
    const err = new Error(
      `TMDB top-rated movies request failed: ${response.status} ${response.statusText} - ${errorText}`
    );
...
// Collect top-rated TMDB movie ids up to the requested limit.
async function fetchTopRatedMovieIds(apiKey, limit) {
  ...
  while (page <= maxPages && collected.length < limit) {
    const { results, totalPages } = await fetchTopRatedMoviePage(apiKey, page);
...
// Ingest top-rated movies while tracking insert and failure stats.
export async function ingestTopRatedMovies({ limit = 20, apiKey } = {}) {
  const resolvedKey = apiKey ?? getApiKey();
  const requestedIds = await fetchTopRatedMovieIds(resolvedKey, limit);
...
async function main() {
  ...
  try {
    try {
      ...
      const result = await ingestTopRatedMovies({ limit });

      console.log(
        `Top-rated movies ingestion complete. Requested: ${result.requested}. ` +
...
if (isDirectRun) {
  main().catch((error) => {
    console.error("Failed to ingest top-rated TMDB movies:", error.message);
    process.exit(1);
  });
}
