...
const SCRIPT_NAME = "inject_popular_shows_today";
const TMDB_POPULAR_SHOWS_URL = "https://api.themoviedb.org/3/tv/popular";
...
// Fetch a single page of popular shows from TMDB.
async function fetchPopularShowPage(apiKey, page) {
  const url = new URL(TMDB_POPULAR_SHOWS_URL);
  ...
  if (!response.ok) {
    ...
    const err = new Error(
      `TMDB popular shows request failed: ${response.status} ${response.statusText} - ${errorText}`
    );
...
// Collect top popular TMDB show ids up to the requested limit.
async function fetchTopPopularShowIds(apiKey, limit) {
  ...
  while (page <= maxPages && collected.length < limit) {
    const { results, totalPages } = await fetchPopularShowPage(apiKey, page);
...
// Ingest today's popular shows while tracking insert and failure stats.
export async function ingestPopularShowsToday({ limit = 20, apiKey } = {}) {
  const resolvedKey = apiKey ?? getApiKey();
  const requestedIds = await fetchTopPopularShowIds(resolvedKey, limit);
  const existingIds = await fetchExistingShowTmdbIds(requestedIds);
  ...
  for (let i = 0; i < requestedIds.length; i += 1) {
    ...
    if (existingIds.has(tmdbId)) {
      console.log(`${prefix} Show ${tmdbId} already exists -> skipped`);
      skippedExisting += 1;
      continue;
    }
    ...
    if (raceCheck.has(tmdbId)) {
      console.log(`${prefix} Show ${tmdbId} exists now -> skipped`);
      skippedRace += 1;
      continue;
    }

    try {
      const result = await ingestTvShow({ tmdbTvId: tmdbId, apiKey: resolvedKey });
      if (result.action !== "inserted") {
        // Keep behavior strict for this loader: treat non-insert as skipped.
        console.log(`${prefix} Show ${tmdbId} already synced elsewhere -> skipped`);
        skippedRace += 1;
        continue;
      }

      inserted += 1;
      console.log(`${prefix} Show ${tmdbId} inserted (id=${result.showId})`);
    } catch (error) {
      failed += 1;
      failedItems.push({ entityType: "show", tmdbId });
      console.warn(`${prefix} Show ${tmdbId} failed ingestion: ${error.message}`);
    }
  }
...
async function main() {
  ...
  try {
    try {
      ...
      const result = await ingestPopularShowsToday({ limit });

      console.log(
        `Popular shows ingestion complete. Requested: ${result.requested}. ` +
...
if (isDirectRun) {
  main().catch((error) => {
    console.error("Failed to ingest popular TMDB shows:", error.message);
    process.exit(1);
  });
}
