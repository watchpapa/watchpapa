...
const SCRIPT_NAME = "inject_changed_shows_24h";
...
export async function ingestChangedShows24h({
  ...
  const existingIds = await fetchExistingShowTmdbIds(changedIds);
  ...
  for (let i = 0; i < toRefresh.length; i += 1) {
    ...
    try {
      ...
      failed += 1;
      console.warn(`${prefix} Show ${tmdbId} failed refresh: ${error.message}`);
    }
  }
    ...
    try {
      ...
      await writeScriptLog({
        ...
        errorDetail:
          result.failed === 0
            ? null
            : `${result.failed} refresh(es) failed in changed shows run.`,
        startedAt,
      });
    } catch (error) {
      await writeScriptLog({
...
if (isDirectRun) {
  main().catch((error) => {
    console.error("Failed to refresh changed TMDB shows:", error.message);
    process.exit(1);
  });
}
