...
const SCRIPT_NAME = "inject_changed_movies_24h";
...
// Refresh existing movies that changed in the selected 24h window.
export async function ingestChangedMovies24h({
  limit = 100000,
  apiKey,
  startDate,
  endDate,
  ...
  const changedIds = await fetchChangedMovieIds({
    apiKey: resolvedKey,
    limit,
    startDate: resolvedRange.startDate,
    endDate: resolvedRange.endDate,
  });
  const existingIds = await fetchExistingMovieTmdbIds(changedIds);
  ...
  for (let i = 0; i < toRefresh.length; i += 1) {
    ...
    if (!raceCheck.has(tmdbId)) {
      console.log(`${prefix} Movie ${tmdbId} no longer exists locally -> skipped`);
      skippedRace += 1;
      continue;
    }
    ...
    try {
      ...
      // Reuse the movie ingest path, forcing updates for existing rows.
      ...
      console.warn(`${prefix} Movie ${tmdbId} failed refresh: ${error.message}`);
    }
  }
...
async function main() {
  ...
  try {
    try {
      ...
      // Execute the changed-movies refresh for the requested date window.
      const result = await ingestChangedMovies24h({ limit, startDate, endDate });
      console.log(
...
if (isDirectRun) {
  main().catch((error) => {
    console.error("Failed to refresh changed TMDB movies:", error.message);
    process.exit(1);
  });
}
