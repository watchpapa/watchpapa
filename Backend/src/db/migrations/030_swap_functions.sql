-- 030_swap_functions.sql — Phase 4, first step of the cutover window.
--
-- Applied minutes before the Worker route flip + Pages deploy. Swaps the three
-- functions whose bigint entity-id argument changes meaning (local id → tmdb_id)
-- and drops the exclusive-or CHECK constraints so the new frontend can insert rows
-- that set only media_type + tmdb_id.
--
-- ROLLBACK (keep this ready during the cutover): re-run 030_swap_functions_down.sql
...
AS $$
  WITH rows AS (
    ...
    WHERE r.media_type = p_media_type AND r.tmdb_id = p_entity_id
  )
...
-- get_observed_ratings_for_entity(text, bigint) — p_entity_id now means tmdb_id.
...
AS $$
  ...
  WHERE o.observer_id = auth.uid()
    ...
    AND r.media_type = p_media_type AND r.tmdb_id = p_entity_id
    AND NOT EXISTS (
...
-- audit_user_follow_change() — log NEW.tmdb_id instead of NEW.movie_id / NEW.show_id.
...
BEGIN
  v_user := COALESCE(
    auth.uid(),
    CASE TG_OP WHEN 'INSERT' THEN NEW.profile_id ELSE OLD.profile_id END
  );

  IF TG_OP = 'INSERT' THEN
    v_method := 'POST';
    IF TG_TABLE_NAME = 'user_followed_movies' THEN
      v_action := 'follow_movie'; v_body := jsonb_build_object('tmdb_id', NEW.tmdb_id);
    ELSIF TG_TABLE_NAME = 'user_followed_shows' THEN
      v_action := 'follow_show';  v_body := jsonb_build_object('tmdb_id', NEW.tmdb_id);
    ELSE RETURN NEW;
    END IF;
  ELSIF TG_OP = 'DELETE' THEN
    v_method := 'DELETE';
    IF TG_TABLE_NAME = 'user_followed_movies' THEN
      v_action := 'unfollow_movie'; v_body := jsonb_build_object('tmdb_id', OLD.tmdb_id);
    ELSIF TG_TABLE_NAME = 'user_followed_shows' THEN
      v_action := 'unfollow_show';  v_body := jsonb_build_object('tmdb_id', OLD.tmdb_id);
    ELSE RETURN OLD;
    END IF;
  ELSE
...
-- Drop the exclusive-or CHECKs — the new frontend writes rows with only
-- media_type + tmdb_id set. (031 adds the replacement CHECKs.)
