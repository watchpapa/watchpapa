-- 031_drop_content.sql — Phase 5, after a clean soak. POINT OF NO RETURN for the
-- droplet rollback: the mirrored TMDB content tables are dropped here.
--
...
-- 2. Delete rows that can never be mapped (the content row was already gone).
--    Review counts first if this is non-zero.
...
-- 3. Lock in the new shape and drop the old id columns (drops FKs + old partial
