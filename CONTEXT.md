...
**Worker** — `wrangler.jsonc` `vars`: `ALLOWED_ORIGINS`, `SUPABASE_URL` (for JWKS), `BATCH_MAX`, `BATCH_SUBREQ_BUDGET`, `IMPORT_CHUNK_MAX`, `RELEASES_MAX_SHOWS`, `SITEMAP_PAGES`, `API_ORIGIN` (used only by the import-job cron safety net to restart a stalled self-fetch chain — see "Import flow"). Secrets (`wrangler secret put`): `TMDB_API_KEY_SECRET`, `INTERNAL_TICK_SECRET` (gates `POST /api/import/_advance/:id` — **required** for background imports to progress past their first chunk). Bindings: `HYPERDRIVE`. `triggers.crons: ["* * * * *"]` (1/min, the Free-plan minimum) drives `scheduled()` → `runImportTick` (background-import safety net, `cron.js`) — no Durable Objects/alarms on Free. Local dev also needs the shell var `CLOUDFLARE_HYPERDRIVE_LOCAL_CONNECTION_STRING_HYPERDRIVE` (= the Supabase pooler URL) + `worker/.dev.vars` with `TMDB_API_KEY_SECRET` and `INTERNAL_TICK_SECRET`.
...
| `POST` | `/api/import/jobs` | JWT + mutation limit + audit | `{uniqueFilms, ratingsSrc, watchlistSrc, watchlistId?, newWatchlistName?, conflictMode}` → inserts an `import_job` row (`pending`), kicks off the self-fetch chain (see "Import flow" below), returns `{jobId}`. No synchronous DB writes to `user_rating`/`watchlist_item` |
| `POST` | `/api/import/_advance/:id` | `X-Internal-Secret` (no JWT) | Internal: the Worker calling itself to advance one job by one chunk, then chaining to the next. Not for client use |
...
Each chunk is advanced by a **real Worker invocation, not a loop inside one invocation** — a Cron Trigger firing once a minute turned out to be both too slow (one chunk/minute — ~27 minutes for a 261-film import) and, in practice, not reliably firing on Workers Builds-deployed projects at all. Instead, `POST /api/import/jobs` kicks off a **self-fetch chain**: it calls `continueChain()` (`worker/src/cron.js`), which does `ctx.waitUntil(fetch(...))` to `POST /api/import/_advance/:id` (mounted directly in `index.js`, gated by the `X-Internal-Secret` header against the `INTERNAL_TICK_SECRET` secret — no user JWT, since it's the Worker calling itself). That handler claims the job (`FOR UPDATE SKIP LOCKED`), advances it one step via `advanceOneStep()` — `pending`/`resolving` resolves the next `IMPORT_CHUNK_MAX` films via `worker/src/import/resolve.js` and flips to `committing` once every film is resolved; `committing` maps `ratingsSrc`/`watchlistSrc` through the accumulated `resolved` map and writes everything in one transaction via `worker/src/import/commit.js`, landing on `done` or `failed` — then, if still active, fires the *next* chunk's invocation the same way. Each chunk gets its own fresh CPU budget (the Free plan's tight ~10ms/request limit is why chunking this way matters — the old synchronous `/resolve` route already proved one chunk per invocation is safe), so a whole import finishes in seconds despite the per-invocation limit. The Cron Trigger (`triggers.crons`, every minute) is now just a safety net: `runImportTick()` sweeps for jobs whose chain died (no progress in 90s) and restarts them via the same `continueChain()`, using the `API_ORIGIN` var since a cron invocation has no incoming request to derive an origin from. **Required for this to work at all:** `INTERNAL_TICK_SECRET` must be set via `wrangler secret put` (local dev: `worker/.dev.vars`) — without it, `continueChain()` logs an error and no-ops, silently stalling every job after its first chunk.
