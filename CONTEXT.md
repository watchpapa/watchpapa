...
| `import_job` | `bigint` identity | `profile_id`, `status` (`pending`\|`resolving`\|`committing`\|`done`\|`failed`\|`cancelled`), `payload` (jsonb: `uniqueFilms`/`ratingsSrc`/`watchlistSrc`/`watchlistId`/`newWatchlistName`/`conflictMode`), `resolved` (jsonb filmKey→tmdbId), `unresolved` (jsonb display strings), `resolve_cursor`, `total`, `result`, `error` | Background Letterboxd/watchpapa import queue (migration 048). Own-read RLS; Worker writes via the pooler role. Advanced by the `worker/src/cron.js` Cron Trigger (1/min — no Durable Objects on Free plan), several `IMPORT_CHUNK_MAX`-film chunks resolved per job per tick, committed once `resolve_cursor >= total`. See "Import flow" below. |
...
Driven entirely by the Cron Trigger (`triggers.crons`, every minute — the Free plan's minimum, no Durable Objects/alarms available). Two earlier designs were tried and abandoned here, worth knowing before changing this again: (1) one chunk resolved per job per cron tick — correct but slow (~27 minutes for a 261-film import); (2) each chunk firing the next one itself via a self-fetch (`ctx.waitUntil(fetch(own url))`) so a whole import would finish in seconds — this **never actually worked in production**: confirmed via `wrangler tail` that the self-chain call landed once but never continued itself past that first hop (likely some Workers-side restriction on a same-zone self-fetch from inside `waitUntil` — not fully diagnosed), so only the periodic cron tick was ever really advancing anything. The current design (`worker/src/cron.js`) is the simpler thing actually proven reliable: `runImportTick()` claims up to `JOBS_PER_TICK` (3) active `import_job` rows per tick (`FOR UPDATE SKIP LOCKED`) and advances each through up to `STEPS_PER_JOB_PER_TICK` (4) chunks in a row within the same tick (stopping early once a job finishes) — `pending`/`resolving` resolves the next `IMPORT_CHUNK_MAX` films via `worker/src/import/resolve.js` per step and flips to `committing` once every film is resolved; `committing` maps `ratingsSrc`/`watchlistSrc` through the accumulated `resolved` map and writes everything in one transaction via `worker/src/import/commit.js`, landing on `done` or `failed`. `POST /api/import/jobs` also calls `kickstartJob()` via `waitUntil` right after inserting the row — one step, in-process, no HTTP call — so the UI shows real progress immediately instead of waiting for the first cron tick. A full import still takes low minutes rather than seconds; going faster than that on the Free plan would need Durable Objects (Workers Paid).
