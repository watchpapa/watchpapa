...
│   │   ├── subscription/             # EarlyAdopterBanner, UpgradePromptToast, TierRewardModal (one-time "you've been upgraded" popup — reads unacked `tier_reward` rows, mounted in AppLayout)
...
| `GET` | `/api/content/list/:kind?page&include_adult` | kind ∈ movies-popular / shows-popular / movies-top-rated / … (see `tmdb/lists.js`). `movies-popular`/`shows-popular` are backed by TMDB `/trending/{movie,tv}/day` (rotates daily, unlike `sort_by=popularity.desc`); NSFW handled by the Worker-side `filterNsfw()` post-filter. `*-top-rated` are **served via `/discover/*`** so `include_adult`, NSFW-keyword exclusion, language and region all apply — TMDB's list endpoints ignore `include_adult` entirely |
...
| `POST` | `/api/import/jobs` | JWT + mutation limit + audit | `{uniqueFilms, ratingsSrc, watchlistSrc, watchlistId?, newWatchlistName?, conflictMode}` → inserts an `import_job` row (`pending`), advances it one step immediately via `waitUntil` (see "Import flow" below), returns `{jobId}`. No synchronous DB writes to `user_rating`/`watchlist_item` |
...
| `POST` | `/api/admin/tier-rewards` | Bulk tier reward — `{tier, durationDays, message?, target:'all'\|'selected', userIds?}` → grants a higher plan (via `apply_tier_upgrade`, upgrade-only) to every eligible active user or a picked set, writes a `tier_reward` ledger row per genuine recipient (drives the one-time popup + a `tier_reward` notification via that table's trigger). One audit row (`admin_tier_reward`, batch id + counts in `body`); the `tier_reward` rows are the per-user trail |
| `GET` | `/api/admin/tier-rewards` | Recent reward batches (grouped by `batch_id`) for the history table |
| `GET` | `/api/admin/tier-rewards/eligible-count?tier=` | `{total, eligible}` active users — blast radius for the "All users" confirm dialog |
...
| `/admin/tier-rewards` | Admin | `TierRewardsPage` | Grant a higher plan to all / picked users at once (tier · days · optional message); recent-batches table. Recipients see `TierRewardModal` + a bell notification |
...
| `tier_reward` | `bigint` identity | `profile_id`, `batch_id`, `tier`, `duration_days` (NULL = lifetime), `expires_at`, `message` (≤280), `granted_by`, `acknowledged_at` | Per-recipient ledger for admin bulk tier rewards (migration 047, `/admin/tier-rewards`). Own-read RLS; no client writes — Worker inserts, `acknowledge_tier_rewards(p_ids)` RPC clears. `AFTER INSERT` trigger `tier_reward_notify()` drops a `tier_reward` notification. `TierRewardModal.jsx` shows unacked rows as a one-time popup. |
...
| `import_job` | `bigint` identity | `profile_id`, `status` (`pending`\|`resolving`\|`committing`\|`done`\|`failed`\|`cancelled`), `payload` (jsonb: `uniqueFilms`/`ratingsSrc`/`watchlistSrc`/`watchlistId`/`newWatchlistName`/`conflictMode`), `resolved` (jsonb filmKey→tmdbId), `unresolved` (jsonb display strings), `resolve_cursor`, `total`, `result`, `error` | Background Letterboxd/watchpapa import queue (migration 048). Own-read RLS; Worker writes via the pooler role. Advanced by the `worker/src/cron.js` Cron Trigger (1/min — no Durable Objects on Free plan); each chunk commits its own `user_rating`/`watchlist_item` rows immediately (see "Import flow" below) rather than one commit at the end, so `result` accumulates across chunks and `committing` is a legacy status only. |
...
Driven entirely by the Cron Trigger (`triggers.crons`, every minute — the Free plan's minimum, no Durable Objects/alarms available). Two earlier designs were tried and abandoned here, worth knowing before changing this again: (1) each chunk firing the next one itself via a self-fetch (`ctx.waitUntil(fetch(own url))`) so a whole import would finish in seconds — this **never actually worked in production**: confirmed via `wrangler tail` that the self-chain call landed once but never continued itself past that first hop (likely some Workers-side restriction on a same-zone self-fetch from inside `waitUntil` — not fully diagnosed), so only the periodic cron tick was ever really advancing anything; (2) processing `JOBS_PER_TICK(3) × STEPS_PER_JOB_PER_TICK(4)` chunks per tick in-process — hit Cloudflare's **50-subrequests-per-invocation** ceiling (each 10-film chunk can cost up to ~20 TMDB fetches) and, worse, treated that as a permanent job failure instead of "just stop this tick, resume next one." Current values (`worker/src/cron.js`): `JOBS_PER_TICK = 1`, `STEPS_PER_JOB_PER_TICK = 2` (worst case ~40 subrequests, safely under 50); a step that throws just breaks the tick's loop without touching the job's status — its cursor/resolved/result from every prior successful step is already durably saved, so the next tick resumes it. Only `commitImport`'s own try/catch (e.g. `WATCHLIST_LIMIT_REACHED`) marks a job genuinely `failed`.

Each resolved chunk **commits its own `ratings`/`watchlistItems` immediately** via `worker/src/import/commit.js` (mapping that chunk's newly-resolved `filmKey`s through `ratingsSrc`/`watchlistSrc`) rather than waiting for a separate final commit — so the profile fills in gradually as the import progresses instead of staying empty until 100% resolved. `commitImport` returns the `watchlistId` it used/created; a chunk that creates a new watchlist persists that id onto `job.payload` so later chunks add to the same list instead of creating a duplicate one each time. `job.result` accumulates across chunks and `resolve_cursor >= total` takes status straight `resolving → done` (no separate `committing` phase for new jobs — a `committing`-status row from before this change is still handled by a legacy fallback branch in `advanceJob`). `POST /api/import/jobs` also calls `kickstartJob()` via `waitUntil` right after inserting the row — one step, in-process, no HTTP call — so the UI shows real progress immediately instead of waiting for the first cron tick. A full import still takes low minutes rather than seconds; going faster than that on the Free plan would need Durable Objects (Workers Paid).
...
| 047 | `tier_rewards` | **Applied 2026-09-08.** New `public.tier_reward` table — the per-recipient ledger for admin bulk tier rewards (`POST /api/admin/tier-rewards`). Columns `profile_id`/`batch_id`/`tier`/`duration_days`/`expires_at`/`message`/`granted_by`/`acknowledged_at`; partial index on unacked rows. Own-read RLS (`profile_id = auth.uid()`), **no** client write policy — the Worker inserts via the pooler role; `acknowledge_tier_rewards(p_ids BIGINT[] DEFAULT NULL)` SECURITY DEFINER RPC (mirrors `mark_notifications_read`) is the only client write, `EXECUTE` to `authenticated` only. Widens `notification_type_check` to allow `'tier_reward'`. `tier_reward_notify()` SECURITY DEFINER `AFTER INSERT` trigger drops one actor-less `tier_reward` notification per reward row. The actual subscription write still goes through `apply_tier_upgrade` (migration 005) — upgrade-only, so recipients already on an equal/higher effective tier are skipped and get no ledger row. New audit action `admin_tier_reward` (`worker/src/auditActions.js`). |

To add the next migration: create `Backend/src/db/migrations/048_<name>.sql`, apply via `mcp__claude_ai_Supabase__apply_migration`.
...
- **Browse:** `GET /api/content/{list/:kind,discover/:type,genres}` — `movies-top-rated`/`shows-top-rated` are **served through `/discover/*`** (`tmdb/discover.js` `discoverParams`/`runDiscover`, shared by `/list/:kind`, `/discover/:type`, and the coming-soon/"available on your services" fetches) rather than TMDB's list endpoints, because those ignore `include_adult` and have no keyword-exclusion param. **`movies-popular`/`shows-popular` are backed by TMDB's `/trending/{movie,tv}/day` feed** (not `/discover?sort_by=popularity.desc` — that sort is a slow-moving average that stays near-identical for days; `/trending` genuinely rotates daily), and fall back to the Worker-side `filterNsfw()` post-filter since `/trending` has no keyword param. Other kinds map straight to a TMDB list path in `lists.js`.
...
**2026-09-08 admin bulk tier rewards + one-time upgrade popup:**

Admins previously could only change one user's tier at a time (`/admin/users`). New `/admin/tier-rewards` page grants a higher plan (Premium/Pro/Pro+) for N days or lifetime, with an optional ≤280-char message, to **all active users** or a **hand-picked set** in one action.

- **Migration 047** — `public.tier_reward` ledger table (one row per genuine recipient), `acknowledge_tier_rewards()` RPC, `notification.type` widened for `'tier_reward'`, `tier_reward_notify()` AFTER INSERT trigger. See Migration History.
- **Worker** `worker/src/routes/admin/tierRewards.js` (`validateIssueBody` is a pure exported helper, unit-tested in `worker/test/tierRewards.test.js`): `POST /` selects eligible recipients (`_tier_rank(get_effective_tier(id)) < _tier_rank(target)`, excludes the acting admin), runs `apply_tier_upgrade(...)` per recipient via `unnest(::uuid[])` and inserts the ledger rows — all in one `sql.begin()`. `apply_tier_upgrade` is upgrade-only, so users already on an equal/higher effective tier are **skipped** (no tier change, no ledger row, no popup) and reported as `skipped` in the response. `GET /` (recent batches, grouped by `batch_id`) + `GET /eligible-count?tier=` (blast-radius estimate) back the page. One audit row per action (`admin_tier_reward`, `batchId`/`granted`/`target`/counts in `body`).
- **Frontend** `pages/admin/TierRewardsPage.jsx` + `features/admin/hooks/useTierRewards.js` (issue form + user picker reusing `/api/admin/users/search` + confirm `Modal` + recent-batches `DataTable`); nav group `"Codes"` → `"Rewards"` in `AdminPage.jsx`. `components/subscription/TierRewardModal.jsx` (mounted in `AppLayout` beside `EarlyAdopterBanner`) reads the recipient's unacknowledged `tier_reward` rows directly via supabase-js (own-read RLS), shows the highest-tier one as a one-time popup (gift glyph, tier + duration/expiry + message, "Got it"), then calls `acknowledge_tier_rewards(null)` + `useCurrentUser().refresh()`. `notificationContent.js` gained a `tier_reward` case returning an `icon` (`GiftIcon`); `NotificationBell`/`NotificationsPage` render that icon instead of an actor initial for actor-less system notifications.
- **No confetti** (deliberate — asked for a restrained modal). The bell notification persists after the popup is dismissed.
- Verified: worker vitest 84 green (11 new), Frontend build + lint clean (no new problems), `tests/rls_tests/tier_reward_rls.test.js` 8 green, and the full grant→upgrade→notify chain confirmed against the live DB inside a rolled-back transaction (`premium`→`pro`, notification row created).

**2026-09-07 app-wide navigation & scroll fixes (3 commits):**

Three long-standing bugs: (1) detail pages opened mid-scroll instead of at the top; (2) browser Back re-mounted the previous page from scratch — scroll lost, infinite-scroll reset to page 1, horizontal poster-row position lost; (3) tab/filter state (Follows tab, Watchlists list, Adult/Search filters) reset to defaults on Back.

- **`components/ScrollManager.jsx`** replaces the old `ScrollToTop.jsx` (mounted once in `main.jsx`, *after* `<App/>`). Sets `history.scrollRestoration = "manual"`; keeps a module `Map<location.key, scrollY>`; on **PUSH** jumps to top pre-paint (`useLayoutEffect`), on **POP** restores the saved offset via `restoreScroll()` (rAF loop + `ResizeObserver(document.body)` retry for ≤1.5 s, aborts on any real user input), on **REPLACE** stays put, and honours `#hash` links. The outgoing entry's position is saved in the layout effect from a continuously-tracked module `lastScrollY` (not `window.scrollY`, which the browser has already clamped by the time the effect runs when the new page is shorter). RR v7 `BrowserRouter` has no `<ScrollRestoration>` (data-router only), hence the hand-roll.
- **`components/ui/PillTabs.jsx`**: the active-tab "scroll into view" now only nudges the tablist's own horizontal `scrollLeft` (via a `listRef` + `getBoundingClientRect` compare) — never `scrollIntoView()`, which was walking up to the window and yanking the whole page down when a tab strip (WhereToWatch regions, PersonPage filmography filters) mounted below the fold. This was the dominant cause of bug #1.
- **`features/content/hooks/useContent.js`**: nulls `data` synchronously on an id/locale key change (render-phase resync) so a detail→detail navigation shows the skeleton, not the previous title at the previous scroll position.
- **Detail pages** (`MoviePage`/`ShowPage`/`PersonPage`/`SeasonPage`/`EpisodePage`/`CollectionPage`): the loading/error `<AppLayout>` branches now get the same fixed-height breadcrumb trail as the loaded branch, so the ~37 px Breadcrumbs bar no longer pops in and shifts content when data resolves.
- **`hooks/useSearchParamState.js`** (new): one URL search param as view state, writes with `{ replace: true }`. Wired into `FollowsPage` (`?tab=`), `WatchlistsPage` (`?list=` + `?sort=`; auto-pick-first only when `?list=` absent, stale param tidied via effect; ids compared as strings for `PillTabs`, numeric id kept for rename/delete), `AdultPage` (`?type=`/`?sort=`/`?cat=`, one combined `setSearchParams` so a type switch can also clear a genre category), `SearchPage` (`?type=`/`?sort=` — now persist across query edits instead of resetting).
- **`lib/listCache.js`** (new): module `Map` of list/browse snapshots, 5-min TTL, 40-entry FIFO cap, in-memory (Sets/nested objects survive). `listKey(name, params)` / `readList(key, ttl?)` / `writeList` / `invalidateList(prefix)`. Keys fold in every discriminator that changes the fetched result (`uid`, `showAdult`, `localeKey`, `providersKey`, `region`, route filters, and a seed signature for recommendation rows). Wired into `useHomeData` (two keys — core rows + a seed-signature key for the Suggested rows), `useMediaBrowse`, `usePeoplePageData`, `useMyServicesPageData` (incl. the `useMergedDiscoverRow` sub-hook via a `cacheKey` prop, and the provider rows), `useAdultPageData`, `useSearch` (opt-in via `{ cache: true }` — SearchPage only, not the navbar typeahead), `useActivityFeed` + `useNotifications` (120 s TTL — "live" surfaces). Each hook: seed `useState` from the snapshot, hydrate-or-fetch on key change (warm ⇒ no network, just a background follow-state refresh), write through whenever settled data changes.
- **`components/home/MediaRow.jsx`** takes a `rowKey` prop (`AppHomePage`/`MediaBrowsePage`/`MyServicesPage` pass stable section keys, not the localised title) and calls **`hooks/useRowScrollRestore.js`** (new) — per-`location.key` × rowKey `Map` of `scrollLeft`, restored on POP with a short rAF retry. `AppHomePage` also skips its `.animate-slide-up` row entrance when `useNavigationType() === "POP"`.
- Verified with Playwright (`/movies`,`/shows`,`/people` → detail → Back): detail opens at top, Back restores vertical + horizontal scroll with **zero `/api/content` refetches**, `?type=`/`?tab=` params survive Back, REPLACE (tab tap) doesn't scroll, no console errors, no `/undefined` links.
...
- **Root cause was TMDB, not our cache.** `movies-popular`/`shows-popular` were served from `/discover/{movie,tv}?sort_by=popularity.desc`, which sorts by TMDB's raw `popularity` float — a slow-moving average whose top ~20 stays near-identical for days. Verified our Worker output was byte-identical to a live TMDB call (the 6 h `cacheTtl` was expiring correctly); `/movie/popular` returns the same static list. `/trending/{movie,tv}/day` genuinely rotates.
- **Fix:** repointed those two list kinds at `/trending/{movie,tv}/day` (`worker/src/tmdb/lists.js` — plain `path`, no `discover` spec). `/trending` has no keyword-exclusion param, so they now rely on the Worker-side `filterNsfw()` post-filter (`adult` flag + title regex) like the other plain-list kinds; `backfillShowStatus()` still runs in the non-discover branch so follow-gating is unaffected. TTL kept at 6 h.
- **Frontend:** `features/home/hooks/useHomeData.js` `popular` no longer re-sorts the merged movie+show list by `tmdb_popularity` (which would undo trending order) — it interleaves the two feeds preserving each one's trending order. `/movies` + `/shows` (`useMediaBrowse.js`) already render server order, no change.
...
- Cross-checked `StaticInfoPages.jsx` (About/Help/Privacy) and `SubscriptionPage.jsx` against the actual shipped redesign (phases 0–8) and fixed everything that had drifted: the Help page's header/nav description (was pre-redesign: Search as a nav link, Collections/My Services as top-level items, a hover-opened profile menu) now matches the real header (Popular/Movies/Shows/People + More, click-only account menu) and gained a section on the phone bottom tab bar, which wasn't documented anywhere; rating controls corrected from "Add new rating"/"Clear rating" to the actual button labels "Change"/"Clear"; the Watched section rewritten for the new chip → popover/sheet "rewatch diary" (was: inline "+" button, expandable list); "Adult tab in header" → "Adult tab in navigation" (matches the settings toggle's real label); every `Settings → X` reference checked against `SettingsPage.jsx`'s actual section list and two wrong ones fixed (`Settings → Data` → `Settings → Your data`, `Settings → Plan` → `Settings → Plan & rewards`). `SubscriptionPage.jsx` was already accurate — no changes needed.
- Privacy's "Security logs" section only described sign-in/follows/referrals/reward-claim logging with a flat 90-day retention — badly stale after migration 044's audit coverage. Rewritten as "Audit and security logs" describing the real scope (ratings, watchlists, favourites, follows/observe, blocks, profile/settings changes, referrals, rewards, admin actions on the account), and retention split honestly: our own `audit_events` table (90 days, verified live against the `audit-events-90d-cleanup` pg_cron job) vs. Supabase's own `auth.audit_log_entries` (its own retention, not controlled by us — empirically confirmed via a live query that it holds 132+ days of history, so no specific number is claimed for it).
- Updated the live "🌍 Streaming, suggestions..." `/updates` announcement (already published, not the archived one) with a new "🎨 A redesign, top to bottom" section — navigation, sign-up, detail pages, settings, profiles, share cards. Admin/audit-log work deliberately left out as internal, not user-facing. Two now-wrong mentions inside the existing post ("Add new rating" → "Change", "Adult tab in the header" → "in the navigation") were also fixed in place.
- Redesigned `MediaShareModal`/`ProfileShareModal` (Phase 7's sweep table listed both but never actually touched either) onto `Modal`/`Button`/`PillTabs`, and enlarged both — the preview was a postage-stamp thumbnail. Rewrote `generateMediaShareCard.js`'s canvas template with a type badge, a proper poster frame (drop shadow + ring), genre pills, a quote-card caption treatment, and a dotted footer matching the profile card; fixed a real bug where long titles were hard-truncated character-by-character with no ellipsis. That redesign introduced its own overflow bug (poster was a fixed multiple of its width regardless of what was drawn below it, so a 2-line title pushed content into the footer) — fixed by computing every below-poster section's height up front and sizing the poster to exactly whatever's left.
- `ContentHero` (home page): the backdrop's fade-to-transparent used the bare `transparent` keyword (`rgba(0,0,0,0)`), which drags the RGB channels toward black as alpha drops — a visible muddy band where the fade was doing its work. Fixed by fading to the same color at alpha 0 instead. Also gave the hero a fixed `min-height` (was: height varied with the featured title's line count) and increased the blur backdrop's overshoot so the blur feather never hits a hard clip.
- `DetailPageLayout`: `html`/`body`'s `overflow-x: hidden` forces the other axis to compute as `auto`, turning them into a scroll container that `position: sticky` anchors to — silently breaks sticky in several browsers (Safari especially). Switched to `overflow-x: clip`, fixing sticky site-wide. The slim title bar that replaces the hero once scrolled past was `lg:hidden` (desktop just lost the title with nothing replacing it) and, being `fixed`, reserved no space for itself — content row and the sticky sidebar's top offset now account for it.
