...
│   ├── features/watchlist/hooks/     # useWatchlistItems (selects tmdb_id, hydrates via useContentBatch; toggleWatched), useItemWatchlistStatus, useWatchedStatus(mediaType,entityId,session) — standalone "mark watched" for MoviePage/ShowPage
...
│   ├── features/home/hooks/          # useHomeData(session, showAdult) — Popular/Suggested/"Popular · On Your Services"/"Suggested · On Your Services"/Coming Soon/Movies/Shows; row set + order + visibility driven by lib/homeRows.js + prefs.homeRowOrder/homeHiddenRows. useSuggestionSeeds(session) — gathers ratings+watched titles as recommendation seeds
│   ├── components/
│   │   ├── watchlist/                # AddToWatchlistButton — auto-adds to first list, toast + checklist picker (multi-list); compact prop for WatchlistsPage; MarkWatchedButton
...
| `POST` | `/api/content/recommendations` | `{items:[{type,id}], exclude:[{type,id}], providers?, watchRegion?}` → `{results, resultsOnMyServices?}` — "Suggested for you" home rows; **never edge-cached** (per-user), `Cache-Control: private` |
...
| `watchlist_item` | `bigint` identity | `watchlist_id` FK, `media_type` ('movie'\|'show'), `tmdb_id`, `watched`, `added_at` | UNIQUE `(watchlist_id, media_type, tmdb_id)`. Rating an item auto-removes it (rated ⇒ opinion formed, no separate `watched` flag needed). `watched` is user-settable independent of rating — see "Mark as watched" below. |
...
To add the next migration: create `Backend/src/db/migrations/036_<name>.sql`, apply via `mcp__claude_ai_Supabase__apply_migration`.
...
- Row names: **"Suggested For You"** and **"Suggested For You · On Your Services"**, both on `/`.
...
- **"Mark as watched" without a rating** — `watchlist_item.watched` existed in the schema since migration 016 but had **no UI anywhere** (confirmed by audit: not even a checkbox on `/watchlists`). New `components/watchlist/MarkWatchedButton.jsx` + `features/watchlist/hooks/useWatchedStatus.js` add a quick action on `MoviePage`/`ShowPage` (alongside Follow/Watchlist) that flips `watched` across every watchlist row for that title, or auto-adds to the user's first list already marked watched if it's on none — same auto-add convention `AddToWatchlistButton` already uses. `WatchlistsPage.jsx`'s `ItemGridCard` also gained a per-row watched toggle (`useWatchlistItems.js` `toggleWatched`) plus a green check badge on the poster. Rating still removes an item from every watchlist entirely (unchanged) — watched is specifically for "seen it, no opinion yet."
