...
> Updated: 2026-04-28 — reflects live DB state after migration 001 + index cleanup
...
| `movie_genre` | `movie_genre_movie_id_genres_id_unique` | ✓ | btree(movie_id, genres_id) | prevents duplicates; enables ON CONFLICT genre upsert |
...
| `show_genre` | `show_genre_show_id_genres_id_unique` | ✓ | btree(show_id, genres_id) | prevents duplicates; enables ON CONFLICT genre upsert |
...
| 2 | `INSERT INTO {movie\|show} (...) ON CONFLICT (tmdb_id) DO UPDATE SET ... RETURNING id` | `{table}_tmdb_id_key` | 1–5 ms |

Endpoint returns after query 2; full ingest runs asynchronously in background.
...
| 2 | `INSERT INTO movie (...) ON CONFLICT (tmdb_id) DO UPDATE SET ... RETURNING id, (xmax=0)` | `movie_tmdb_id_key` | 2–10 ms |
| 3 | `DELETE FROM movie_genre WHERE movie_id = :movieId` | `movie_genre_movie_id_genres_id_unique` | <1 ms |
| 4 | `SELECT id, tmdb_id FROM genres WHERE tmdb_id IN (:gid0, …)` *(all genres in one query)* | `genres_tmdb_id_key` | <1 ms |
| 5 | `INSERT INTO movie_genre (movie_id, genres_id) VALUES (…), (…)` *(bulk)* | — | <1 ms |
| 6 | `DELETE FROM movie_credits WHERE movie_id = :movieId` | `movie_credits_movie_id_idx` | <1 ms |
| 7 | `SELECT id, tmdb_id FROM person WHERE tmdb_id IN (:eid0, …)` *(all persons in one query)* | `person_tmdb_id_key` | 1–3 ms |
| 8 | `INSERT INTO person (…) VALUES (…), (…) … ON CONFLICT (tmdb_id) DO NOTHING RETURNING id, tmdb_id` *(bulk)* | `person_tmdb_id_key` | 2–10 ms |
| 9 | `INSERT INTO person_aka (person_id, nickname) VALUES (…), (…)` *(bulk, new persons only)* | — | 1–5 ms |
| 10×J | `SELECT j.id FROM job … WHERE LOWER(j.name) = LOWER(:jobName)` *(per unique job name, cached in Map)* | `job_name_lower_idx` | <1 ms each |
| 11 | `INSERT INTO movie_credits (movie_id, person_id, job_id, title) VALUES (…), (…)` *(bulk)* | — | 1–5 ms |
| 12 | `INSERT INTO script_logs (…)` | — | <1 ms |

**Total: ~12 fixed queries + J unique job name lookups** (J typically 5–15, hit DB once each then cached). Previously 215–600+.

---

### Inject TV Show (full ingest, inside transaction)

Includes everything from movie injection, plus per-season and per-episode work:

| # | Query | Index used | Est. time |
|---|-------|-----------|-----------|
| S1 per season | `INSERT INTO season (...) ON CONFLICT (tmdb_id) DO UPDATE SET ... RETURNING id` | `season_tmdb_id_key` | 1–3 ms |
| S2 per episode | `INSERT INTO episode (...) ON CONFLICT (tmdb_id) DO UPDATE SET ... RETURNING id` | `episode_tmdb_id_key` | 1–3 ms |
| S3 per episode | `DELETE FROM episode_credits WHERE episode_id = :episodeId` | `episode_credits_episode_id_idx` | <1 ms |
| S4 per episode | Batch SELECT + bulk INSERT for persons not yet seen in prior episodes | `person_tmdb_id_key` | 1–5 ms |
...
| 5×A | Insert / restore / soft-delete AKA rows (per AKA delta) | — | <1 ms each |
...
| **Movie** | 150–600+ | ~12–20 | Per-person loop (4–5q each) → 3 bulk queries total |
...
| Existence check | 1 |
| Upsert entity | 1 |
| Genre replace (DELETE + batch SELECT + bulk INSERT) | 3 |
| Delete old credits | 1 |
| Batch SELECT all persons | 1 |
...
| Batch genre lookup (1 SELECT + 1 bulk INSERT) | ✅ | `inject_movie.js`, `inject_tv_show.js` |
| Batch person ingestion (`batchIngestPersons`) | ✅ | `inject_person.js`, `inject_movie.js`, `inject_tv_show.js` |
...
|------------|---------|-------|
| Genre upsert using new unique constraint | Avoids DELETE on every ingest; preserves `created_at` history | `movie_genre` and `show_genre` now have `UNIQUE(movie_id/show_id, genres_id)` — enables `INSERT … ON CONFLICT DO NOTHING` + targeted delete of removed genres only |
